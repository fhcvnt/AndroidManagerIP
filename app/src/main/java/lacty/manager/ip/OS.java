...
import lacty.manager.ip.databinding.ActivityOsBinding;

public class OS extends AppCompatActivity {
    ActivityOsBinding binding;
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId() == R.id.menuitemnormalAdd) {
            ...
            intent.setClass(getApplicationContext(), OSAdd.class);
            ...
            if (!select_view_id.getText().toString().trim().isEmpty()) {
                String delete = "DELETE HeDieuHanh WHERE MaHeDieuHanh='" + select_view_id.getText().toString() + "'";
                int result = connect.execUpdateQuery(delete);
                if (result > 0) {
                    try {
                        // cap nhat lai bang du lieu sau khi xoa
                        binding.tableOS.removeView(row_select);
            ...
            if (select_view != null) {
                Intent intent = new Intent();
                intent.setClass(getApplicationContext(), OSEdit.class);
    ...
    public void sqlSearch(String chuoi) {
        ...
        if (connection.getConnection() != null) {
            ...
            try {
                // so thu tu
                int stt = 1;
                String select = "SELECT MaHeDieuHanh,HeDieuHanh FROM HeDieuHanh WHERE HeDieuHanh LIKE N'%" + chuoi + "%' OR dbo.convertUnicodetoASCII(HeDieuHanh) LIKE N'%" + chuoi + "%' ORDER BY MaHeDieuHanh ASC";
                ...
                while (resultSet.next()) {
                    ...
                    binding.tableOS.addView(tr, new TableLayout.LayoutParams(TableRow.LayoutParams.MATCH_PARENT, TableRow.LayoutParams.WRAP_CONTENT));
                    stt++;
                }
                resultSet.close();
    ...
    private View.OnClickListener onClickListenerData(View view, TextView view_id, TextView view_os, TableRow row) {
    ...
    public void clearTable() {
        int countrow = binding.tableOS.getChildCount();
        binding.tableOS.removeViews(1, countrow - 1);
    }
}